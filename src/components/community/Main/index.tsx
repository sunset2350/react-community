...
import { Maincontainer, ContentTitle, ImageItem, ViewDetailsLink } from './styles';
...
function MediaElement({ contentType, uuidFileName }: { contentType: string; uuidFileName: string }) {
    if (contentType.includes('image')) {
        return (
            <img width={300} height={300} src={`http://localhost:8080/community/files?uuidFilename=${uuidFileName}`} />
        );
    } else {
        return (
            <video controls>
...
const Main = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const response = await axios.get('http://localhost:8080/community/posts');
